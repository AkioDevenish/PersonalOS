...
/**
 * The time ledger's way in from the phone.
 *
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(await convex.query(api.time.ledger, range))
  } catch (error) {
    console.error("[time] read failed:", error)
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      await convex.mutation(api.time.add, {
    ...
    console.error("[time] write failed:", error)
...
export async function DELETE(request: Request) {
  ...
  try {
    const convex = getConvexClient(bearer(request) || caller.token)
    return NextResponse.json(
      await convex.mutation(api.time.remove, { id: id as never })
    )
  } catch (error) {
    console.error("[time] delete failed:", error)
